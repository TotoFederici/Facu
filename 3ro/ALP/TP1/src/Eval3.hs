...
lookfor v m= case M.lookup v (Prelude.fst m) of
  (Just a) -> Right a 
...
update v n (m,s) = (M.insert v n m,s)
...
stepComm (Let v n) m = case evalExp n m of
  Right (x :!: m') ->  Right (Skip :!: (update v x (addTrace ("Let " ++ v ++" "++ show x ++ " " ) m')))
  Left e -> Left e
...
evalExp (VarInc v) m = case lookfor v m of
  Right n  -> Right ( n+1 :!:(update v (n+1) (addTrace ("Let " ++ v ++ " " ++ show (n+1) ++ " ") m)))
  Left e -> Left e 
