...
-- IMPLEMENTACION DE CASE EN PPRINTER , (El pattern lo toma como un IfThenElse)


-- pComm (Case (xs)) = text "case" <+> lbrace $$ (pCase xs) 
-- pCase :: [(Exp a , Comm)] -> Doc
-- pCase [] = rbrace
-- pCase ((b,c):xs) = parens (pExp b) <+> text ":" <+> lbrace <+>  (pComm c) <+> rbrace $$  (pCase xs)  
