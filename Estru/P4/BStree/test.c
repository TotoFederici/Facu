...
void imprimir_int(void* a, void* b){
	printf("%d ", *(int*)a);
}
...
int main() {
  ...
  BSTree tree1 = bstee_crear();
  for (int i = 0; i < N_PALABRAS; i++)
    tree1 = bstree_insertar(tree1, palabras[i], copiar_cadena, comparar_cadena);

  printf("Recorrido inorden: \n");
  bstree_recorrer(tree1, BTREE_RECORRIDO_IN, imprimir_cadena, NULL);
  puts("");
	...
	printf("Es BSTree?: %d\n", validar(tree1, comparar_cadena));
	
  // Destruir arbol
  bstree_destruir(tree1, destruir_cadena);
