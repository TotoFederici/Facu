...
toIfThenElse :: [((Exp Bool),Comm)] -> Comm
toIfThenElse [] = Skip
toIfThenElse ((b,c):xs) = IfThenElse b c (toIfThenElse xs)

vIfThenElse :: Comm -> Maybe Comm
vIfThenElse a@(IfThenElse _ _ _) = Just a
vIfThenElse _ = Nothing 

toCase :: Comm -> [(Exp Bool,Comm)]
toCase (IfThenElse a b Skip) = [(a,b)]
toCase (IfThenElse a b rest) = (a,b):(toCase rest)
toCase _ = []


pattern Case :: [(Exp Bool, Comm)] -> Comm
pattern Case xs <- (toCase -> xs)  where
  Case xs = toIfThenElse xs
